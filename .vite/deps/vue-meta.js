...
function x(t) {
  if (g(t)) return t.map(x);
  if (S(t)) {
    const e = {};
    for (const o in t) e[o] = "context" === o ? t[o] : x(t[o]);
    return e;
  }
  return t;
}
var O = (t, e, o) => {
  const r = [];
  for (const n of t) n && e in n && (r.push(n[e]), o && o(n));
  return r;
};
var $ = (t, e = [], o, r) => {
  if (!o && !r && ({ active: o, sources: r } = t, e.length)) for (let t2 = 0; t2 < e.length; t2++) {
    const n2 = e[t2];
    if (!o || !o[n2]) return void console.error(`recompute: segment ${n2} not found on target`, e, o);
  ...
  const n = ((t2, ...e2) => {
    ...
    if (e2) {
      for (const t3 of e2) if (t3 && S(t3)) for (const e3 in t3) o2.includes(e3) || o2.push(e3);
    }
    return o2;
  })(...r), s = Object.keys(o);
  for (const t2 of s) n.includes(t2) || delete o[t2];
  for (const s2 of n) {
    let n2 = false;
    ...
    if (n2) {
      o[s2] || (o[s2] = {});
      const n3 = [];
      for (const t2 of r) s2 in t2 && n3.push(t2[s2]);
      $(t, [...e, s2], o[s2], n3);
      continue;
    }
    !o[s2] && g(r[0][s2]) && (o[s2] = []);
...
function F(t, o, r, n) {
  return "attributesFor" in n ? (function(t2, o2, r2, n2) {
    ...
    if (!s || !r2) return;
    if (t2.isSSR) return { to: "", vnode: h(`ssr-${s}`, r2) };
    if (!M[s]) {
      const [t3, e] = Array.from(document.querySelectorAll(s));
      if (!t3) return void console.error("Could not find element for selector", s, ", won't render attributes");
    ...
    for (const t3 of a) i.removeAttribute(t3);
  })(t, o, r, n) : "group" in n ? (function(t2, e, o2, r2) {
    if (g(o2)) return console.warn("Specifying an array for group properties isnt supported"), [];
    return Object.keys(o2).map(((n2) => {
      const s = { group: e, data: o2 };
      if (r2.namespaced) s.tagNamespace = true === r2.namespaced ? e : r2.namespaced;
...
function E(t, o, r, n = {}, s) {
  const i = ["content", "json", "rawContent"], c = (t2) => (function(t3, e) {
    for (const o2 of t3) {
      const t4 = h2[o2];
      if (o2 && t4) return t4[e];
    }
  })([a, n.tag], t2);
  if (g(r)) return r.map(((e) => E(t, o, e, n, s))).filter(Boolean).flat();
  const { tag: a = n.tag || o } = r;
  let u = "", l = false, f = false;
  if (v(r)) u = r;
  else if (r.children && g(r.children)) l = true, u = r.children.map(((e) => {
  ...
  let { attrs: m2 } = r;
  if (m2 || "object" != typeof r) m2 || (m2 = {});
  else {
    m2 = { ...r }, delete m2.tag, delete m2.children, delete m2.to;
    for (const t2 of i) delete m2[t2];
  }
  if (l) u = U(t, p2, u, r);
  else {
  ...
  if ("title" === b2 && !t.isSSR) return void (document.title = u);
  f && u && (m2.innerHTML = u);
...
function L(t, e, o) {
  for (const r in e) r in o ? S(t[r]) ? L(t[r], e[r], o[r]) : e[r] !== o[r] && (t[r] = e[r]) : t[r] = e[r];
  for (const r in o) e && r in e || delete t[r];
}
function q(t) {
  if (t || (t = getCurrentInstance() || void 0), t) return t.appContext.config.globalProperties.$metaManager;
}
function B(t, e) {
  const s = getCurrentInstance() || void 0;
  if (!e && s && (e = q(s)), !e) throw new Error("No manager or current instance");
...
var T = defineComponent({ name: "Metainfo", inheritAttrs: false, setup: (t, { slots: e }) => () => {
  const t2 = q();
  if (t2) return t2.render({ slots: e });
} });
function D(t, e, o, r) {
  const n = g(r) ? r : [r];
...
var J = class _J {
    ...
    if (r) {
      ...
      if (n && n.offsetParent) {
        let r2 = new MutationObserver(((s) => {
          for (const { removedNodes: i } of s) i && i.forEach(((s2) => {
  ...
  render({ slots: t } = {}) {
    ...
    for (const e in o) {
      ...
      for (const { to: t2, vnode: e2 } of i) D(this.isSSR, n, t2 || c || "head", e2);
    }
    if (t) for (const e in t) {
      const r2 = "default" === e ? "head" : e;
      if ("head" !== r2 && "body" !== r2) continue;
      const s = t[e];
